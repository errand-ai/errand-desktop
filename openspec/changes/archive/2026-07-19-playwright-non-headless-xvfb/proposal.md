...
- **Bundled resource fix**: bump task-runner container from 256 MiB/1 CPU to 8 GiB/2 CPUs. With the non-headless Playwright flow, task-runner holds the MCP tool catalog, an LLM-context buffer (16 k output tokens), and an open browser context — 256 MiB OOM-kills the runner (exit 137) before it can write `/output/result.json`. The limit was raised in stages as multi-turn browser sessions kept hitting exit 137 (256 MiB → 2 GiB → 8 GiB); 8 GiB clears 35B-class models with multi-turn browser tool use. This was originally a separate PR (#15) and was bundled into this branch by request.
...
- **Resource usage**:
  ...
  - Task-runner container: **256 MiB → 8 GiB memory, 1 → 2 CPUs**. Each concurrently-running task now reserves 8 GiB; previously 256 MiB. Operators on Apple Containerization should size accordingly — N concurrent tasks ≈ N × 8 GiB of VM-allocated memory in addition to the long-running services.
